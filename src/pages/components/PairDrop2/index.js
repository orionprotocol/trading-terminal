      ...
      auxTab.map((res, key) => {
        ...
        if (res === 'FAVS') {
          if (currentQuote === 'FAVS') {
            QUOTE = <i className="fas fa-star"></i> ;//active star
          } else {
            QUOTE = <i className="far fa-star"></i>;//inactive
          }
        }
        return (
          <span
...
/*   console.log(History) */
  const handlePair = (symbolA, symbolB) => {
    ...
    setSymbolA(symbolA);
    setSymbolB(symbolB);
    History.push(`/trade/${symbolA}_${symbolB}`);
  ...
  return (
    /* pair-drop js-pair-drop */
    <div className={`responsive-pair-drop ${mode}`}>
      <div className="titles">{assetsRender}</div>

      <div className="search">
       
        <input type="text" placeholder="Search pair" onChange={handleChange} value={searcher} />
