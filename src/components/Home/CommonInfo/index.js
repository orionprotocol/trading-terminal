...
const CommonInfo = ({ History }) => {
    /* REDUX */
    const { symbolA, symbolB, lastPrice, high, low, vol, change, tickers, supportTradingPairs } = useSelector(
        state => state.general
    );
        ...
        _ => {
            if (symbolB === 'USDT') {
                ...
                if (tickers[`${symbolB}-USDT`] !== undefined) {
                    ...
                    let sizeOfNumber = v.toString().replace('.', '').length - 3
                    if (v <= 999) {
                        v=`${v}`
                    } else if (v > 999 && v <= 999999) {
                        v=`${(v / Math.pow(10, sizeOfNumber)).toFixed(2)} K`
                    } else if (v > 999999 && v <= 999999999) {
                        v=`${(v / Math.pow(10, sizeOfNumber)).toFixed(2)} M`
                    } else if (v > 999999999) {
                        v=`${(v / Math.pow(10, sizeOfNumber)).toFixed(2)} B`
                    }
    ...
    useEffect(() => {
        ...
        if (finalvalue <= 999) {
            setformatVol(`${finalvalue}`)
        } else if (finalvalue > 999 && finalvalue <= 999999) {
            setformatVol(`${(finalvalue / Math.pow(10, sizeOfNumber)).toFixed(2)} K`)
        } else if (finalvalue > 999999 && finalvalue <= 999999999) {
            setformatVol(`${(finalvalue / Math.pow(10, sizeOfNumber)).toFixed(2)} M`)
        } else if (finalvalue > 999999999) {
            setformatVol(`${(finalvalue / Math.pow(10, sizeOfNumber)).toFixed(2)} B`)
        }
    }, [vol]);
