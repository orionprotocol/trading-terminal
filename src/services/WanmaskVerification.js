    ...
    new Promise((resolve, reject) => {
        window.wan3.eth.getAccounts((err, accounts) => {
            if (err != null) {
                ...
                // En este caso wanmask esta instalado pero el cliente no esta logeado
                // console.log('WanMask is locked');
                resolve(true);
            } else {
                // En este caso wanmask esta instalado y el cliente esta logeado
...
const WanmaskVerification = props => {
    ...
    const setWanActive = useCallback(
        payload => dispatch({ type: 'SetWanActive', payload }),
        [dispatch]
    );
